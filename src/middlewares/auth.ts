...
const authMiddleware = async function (req: Request, _res: Response, next: NextFunction) {
  if (req.path === '/auth') return next()
  if (req.method === 'POST' && req.path === '/v1/users') return next()
  if (req.method === 'POST' && req.path === '/v1/animes') return next()

  if (!req.headers.authorization) return next(new AuthenticationError('missing authorization token'))
  ...
  if (user instanceof UserNotFoundError) return next(new AuthenticationError('user not found'))
