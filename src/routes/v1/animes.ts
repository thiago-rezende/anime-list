...
import {
  ...
  listAnimes,
  updateAnime
} from '~/controllers/animes';
import { animesView, animeView } from '~/views/animes';

import {
  AnimeCreationError,
  InvalidAnimeRequestBodyError
} from '~/errors/anime';
...
animes.get(
  '/',
  async (req: Request<{}, {}, {}, ListAnimesRequestQuery>, res: Response) => {
...
animes.get(
  '/:slug',
  async (
    req: Request<GetAnimeRequestParms>,
    res: Response,
    next: NextFunction
  ) => {
...
animes.post(
  '/',
  async (
    req: Request<{}, {}, CreateAnimeRequestBody>,
    res: Response,
    next: NextFunction
  ) => {
    const reqAnime = req.body.anime;

    const invalidCreateAnimeRequest = new InvalidAnimeRequestBodyError(
      'invalid create anime request body',
      []
    );

    if (!reqAnime) {
      invalidCreateAnimeRequest.fields.push({
        field: 'anime',
    ...
    if (!name || !slug || !native || !romaji || !synopsis || !releaseDate) {
      if (!name)
        invalidCreateAnimeRequest.fields.push({
          field: 'name',
          description: "the anime object should have an attribute 'name'"
        });
      if (!slug)
        invalidCreateAnimeRequest.fields.push({
          field: 'slug',
          description: "the anime object should have an attribute 'slug'"
        });
      if (!native)
        invalidCreateAnimeRequest.fields.push({
          field: 'native',
          description: "the anime object should have an attribute 'native'"
        });
      if (!romaji)
        invalidCreateAnimeRequest.fields.push({
          field: 'romaji',
          description: "the anime object should have an attribute 'romaji'"
        });
      if (!synopsis)
        invalidCreateAnimeRequest.fields.push({
          field: 'synopsis',
          description: "the anime object should have an attribute 'synopsis'"
        });
      if (!releaseDate)
        invalidCreateAnimeRequest.fields.push({
          field: 'releaseDate',
          description: "the anime object should have an attribute 'releaseDate'"
        });
...
animes.delete(
  '/:id',
  async (
    req: Request<DeleteAnimeRequestParams>,
    res: Response,
    next: NextFunction
  ) => {
...
animes.patch(
  '/:id',
  async (
    req: Request<UpdateAnimeRequestParams, {}, UpdateAnimeRequestBody>,
    res: Response,
    next: NextFunction
  ) => {
    const reqAnime = req.body.anime;

    const invalidUpdateAnimeRequestBody = new InvalidAnimeRequestBodyError(
      'invalid update anime request body',
      []
    );

    if (!reqAnime) {
      invalidUpdateAnimeRequestBody.fields.push({
        field: 'anime',
        description: "should have an object 'anime'"
      });
