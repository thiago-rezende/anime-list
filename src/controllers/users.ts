...
export async function listUsers(
  options?: FindOptions
): Promise<{ rows: Array<User>; count: number }> {
...
export async function deleteUser(
  id: string
): Promise<UserNotFoundError | void> {
...
export async function updateUser(
  id: string,
  data: UserDTO
): Promise<UserNotFoundError | User> {
  const user = await User.findByPk(id);
